...
class TransactionTicketAdmin(admin.ModelAdmin):
    ...
    def checkin_view(self, request, object_id, form_url='', extra_context=None):
        ...
        if not (request.user.is_superuser or group_name == TICKET_CONDUCTOR or group_name == TICKET_DIRECTOR):
            raise PermissionDenied
        ...
        self.readonly_fields = ['ticket_bank', 'number', 'amount', 'receive_ems', 'status', 'create_time', 'finish_time']
        self.exclude = ['transaction', ]
        self.inlines = []
        return super(TransactionTicketAdmin, self).change_view(request, object_id, form_url, extra_context)
    ...
    def change_view(self, request, object_id, form_url='', extra_context=None):
        if request.user.is_superuser:
            ...
            return super(TransactionTicketAdmin, self).change_view(request, object_id, form_url, extra_context)
        self.readonly_fields = ['number', 'ticket_bank', 'amount', 'receive_ems', 'send_ems', 'status', 'create_time', 'finish_time']
        self.exclude = ['transaction']
        self.inlines = [TicketLogInline, ]
        ...
        if group_name in (TICKET_CONDUCTOR, TICKET_DIRECTOR, TOP_MANAGER):
            pass
        else:
            return Http404
    ...
    def changelist_view(self, request, extra_context=None):
        if request.user.is_superuser:
            self.list_filter = ['status', ]
            self.list_display = ['number', 'transaction_link', 'ticket_bank_link', 'amount', 'receive_ems', 'send_ems', 'create_time', 'receive_ticket_link', 'verify_ticket_link',
                                 'checkin_link', 'checkout_link']
            return super(TransactionTicketAdmin, self).changelist_view(request, extra_context)

        user_profile = get_user_profile(request.user)
        group_name = None if user_profile is None else user_profile.groupname

        if group_name == TICKET_CONDUCTOR:
            self.list_filter = []
            self.list_display = ['number', 'transaction_link', 'ticket_bank_link', 'amount', 'receive_ems', 'send_ems', 'create_time', 'receive_ticket_link', 'verify_ticket_link',
                                 'checkin_link', 'checkout_link']
            if 'verify_pending' in request.GET and request.GET['verify_pending'].isdigit():
                ...
                ticketlog.after_status = TICKET_VERIFIED_PENDING
                ticketlog.remarks = u'验票操作，等待审核'
                ticketlog.operator = user_profile
                ticketlog.save()
                ticket.status = TICKET_VERIFIED_PENDING
                ticket.save()

            # if 'checkin_pending' in request.GET and request.GET['checkin_pending'].isdigit():
            #     ticket = TransactionTicket.objects.get(pk=long(request.GET['checkin_pending']))
            #     ticket.status = TICKET_CHECKIN_PENDING
            #     ticket.save()
            ...
            if 'checkout_pending' in request.GET and request.GET['checkout_pending'].isdigit():
                ...
                ticketlog.ticket = ticket
                ticketlog.before_status = ticket.status
                ticketlog.after_status = TICKET_CHECKOUT_PENDING
                ticketlog.remarks = u'出库操作，等待审核'
                ticketlog.operator = user_profile
                ticketlog.save()
                ticket.status = TICKET_CHECKOUT_PENDING
                ticket.save()

        elif group_name == TICKET_DIRECTOR:
            self.list_filter = []
            self.list_display = ['number', 'transaction_link', 'ticket_bank_link', 'amount', 'receive_ems', 'send_ems', 'create_time', 'receive_ticket_link_director',
            ...
            if 'receive_confirm' in request.GET and request.GET['receive_confirm'].isdigit():
                ...
                ticketlog.ticket = ticket
                ticketlog.before_status = ticket.status
                ticketlog.after_status = TICKET_RECEIVED
                ticketlog.remarks = u'收票审核通过'
                ticketlog.operator = user_profile
                ticketlog.save()
                ticket.status = TICKET_RECEIVED
                ticket.save()
            ...
            if 'verify_confirm' in request.GET and request.GET['verify_confirm'].isdigit():
                ...
                ticketlog.ticket = ticket
                ticketlog.before_status = ticket.status
                ticketlog.after_status = TICKET_VERIFIED
                ticketlog.remarks = u'验票审核通过'
                ticketlog.operator = user_profile
                ticketlog.save()
                ticket.status = TICKET_VERIFIED
                ticket.save()
            ...
            if 'checkin_confirm' in request.GET and request.GET['checkin_confirm'].isdigit():
                ...
                ticketlog.ticket = ticket
                ticketlog.before_status = ticket.status
                ticketlog.after_status = TICKET_CHECKIN
                ticketlog.remarks = u'入库审核通过'
                ticketlog.operator = user_profile
                ticketlog.save()
                ticket.status = TICKET_CHECKIN
                ticket.save()
            ...
            if 'checkout_confirm' in request.GET and request.GET['checkout_confirm'].isdigit():
                ...
                ticketlog.ticket = ticket
                ticketlog.before_status = ticket.status
                ticketlog.after_status = TICKET_CHECKOUT
                ticketlog.remarks = u'出库审核通过'
                ticketlog.operator = user_profile
                ticketlog.save()
                ticket.status = TICKET_CHECKOUT
                ticket.save()
        else:
            raise PermissionDenied
